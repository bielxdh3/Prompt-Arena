...
Status: implementation and verification in progress. Static, rendered, and native local UI evidence is recorded; privileged clean-install/uninstall and live text-generation gates remain open. Do not treat this draft as owner acceptance.
...
- Ollama and LM Studio transport/listing/persistence were exercised live; llama.cpp was not available on this host, and successful text generation plus Windows DPI checks remain unverified.
