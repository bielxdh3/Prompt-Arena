...
- Rust command, worker, path-safety, and migration contract tests: `cargo test --manifest-path src-tauri/Cargo.toml --all-targets`.
...
There is no live SQLite executor, artifact byte writer, app-spawned worker lifecycle, model runtime, benchmark fixture,
or historical-data migration test. Those checks belong with the phase that implements each behavior; the current tests
verify the contracts without inventing production records.
