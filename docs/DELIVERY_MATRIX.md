...
- A fresh normal-GPU retest of product SHA `f3048a61f45cdb6b29f362444f3d32646edf4943` in an isolated clone, using verified tauri-driver `2.0.6` and EdgeDriver `151.0.4129.101`, failed at WebDriver `POST /session` with `session not created from chrome not reachable`. DevToolsActivePort existed, but WebView2 GPU exited repeatedly with `STATUS_ACCESS_DENIED (0xC0000022)`; no Windows acceptance marker was produced and bridge/Rust/sidecar/persistence/reopen assertions were not exercised. Classify this as `LOCAL_WINDOWS_WEBVIEW2_GPU_HOST_LIMITATION`, not product acceptance.
- An authorized B-only A/B retest set `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS=--disable-gpu` only in the child test process; it failed with the same `session not created from chrome not reachable` error and GPU access-denied diagnostic. The flag was cleared after the child process and was not persisted; this remains `LOCAL_WINDOWS_WEBVIEW2_GPU_HOST_LIMITATION`, not a product finding.
- During B, the environment reported Edge ProductVersion `152.0.4191.53`, while WebView2 and the supplied EdgeDriver remained `151.0.4129.x`. This is an unproven environmental hypothesis, not a product finding.
- Native Windows acceptance remains unproven; no merge, release, tag, or deploy occurred.
